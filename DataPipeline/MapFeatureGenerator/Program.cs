...
public static class Program
{
        ...
        foreach (var (tileId, _) in mapData.Tiles)
        {
            ...
            foreach (var t in featureIds)
            {
                ...
                for (var i = 0; i < featureData.PropertyKeys.keys.Count; ++i)
                {
                    ...
                    if (SubPropToEnumCode(v) != GroundSubType.Unknown)
                    {
                        fileWriter.Write(stringOffset);
                        fileWriter.Write(1);
                        stringOffset += 1;
    ...
    private static GroundType PropToEnumCode(string propName)
    {
        var dict = new Dictionary<string, GroundType>
        {
            { "admin_level", GroundType.AdminLevel },
            { "place", GroundType.Place },
            { "name", GroundType.Name },
            { "highway", GroundType.Highway },
            { "water", GroundType.Water },
            { "railway", GroundType.Railway },
            { "natural", GroundType.Natural },
            { "boundary", GroundType.Boundary },
            { "landuse", GroundType.Landuse },
            { "building", GroundType.Building },
            { "amenity", GroundType.Amenity },
            { "leisure", GroundType.Leisure }
        };
    ...
    private static GroundSubType SubPropToEnumCode(string subPropName)
    {
        var dict = new Dictionary<string, GroundSubType>
        {
            { "motorway", GroundSubType.Motorway },
            { "trunk", GroundSubType.Trunk },
            { "primary", GroundSubType.Primary },
            { "secondary", GroundSubType.Secondary },
            { "tertiary", GroundSubType.Tertiary },
            { "unclassified", GroundSubType.Unclassified },
            { "road", GroundSubType.Road },
            { "natural", GroundSubType.Natural },
            { "fell", GroundSubType.Fell },
            { "grassland", GroundSubType.Grassland },
            { "heath", GroundSubType.Heath },
            { "moor", GroundSubType.Moor },
            { "scrub", GroundSubType.Scrub },
            { "wetland", GroundSubType.Wetland },
            { "wood", GroundSubType.Wood },
            { "tree_row", GroundSubType.TreeRow },
            { "bare_rock", GroundSubType.BareRock },
            { "rock", GroundSubType.Rock },
            { "scree", GroundSubType.Scree },
            { "beach", GroundSubType.Beach },
            { "sand", GroundSubType.Sand },
            { "water", GroundSubType.Water },
            { "2", GroundSubType.Two },
            { "city", GroundSubType.City },
            { "town", GroundSubType.Town },
            { "locality", GroundSubType.Locality },
            { "hamlet", GroundSubType.Hamlet },
            { "administrative", GroundSubType.Administrative },
            { "forest", GroundSubType.Forest },
            { "orchard", GroundSubType.Orchard },
            { "residential", GroundSubType.Residential },
            { "cemetery", GroundSubType.Cemetery },
            { "industrial", GroundSubType.Industrial },
            { "commercial", GroundSubType.Commercial },
            { "square", GroundSubType.Square },
            { "construction", GroundSubType.Construction },
            { "military", GroundSubType.Military },
            { "quarry", GroundSubType.Quarry },
            { "brownfield", GroundSubType.Brownfield },
            { "farm", GroundSubType.Farm },
            { "meadow", GroundSubType.Meadow },
            { "grass", GroundSubType.Grass },
            { "greenfield", GroundSubType.Greenfield },
            { "recreation_ground", GroundSubType.RecreationGround },
            { "winter_sports", GroundSubType.WinterSports },
            { "allotments", GroundSubType.Allotments },
            { "reservoir", GroundSubType.Reservoir },
            { "basin", GroundSubType.Basin }
        };

        if (dict.ContainsKey(subPropName))
            return dict[subPropName];

        return GroundSubType.Unknown;
    }

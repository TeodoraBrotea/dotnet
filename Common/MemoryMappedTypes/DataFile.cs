...
public readonly ref struct MapFeatureData
{
    ...
    public Dictionary<GroundType, Tuple<string, GroundSubType>> Properties { get; init; }
}
    ...
    public void ForeachFeature(BoundingBox b, MapFeatureDelegate? action)
    {
        ...
        for (var i = 0; i < tiles.Length; ++i)
        {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j)
            {
                ...
                ReadOnlySpan<char> label = new ReadOnlySpan<char>();
                ...
                if (isFeatureInBBox)
                {
                    var properties = new Dictionary<GroundType, Tuple<string, GroundSubType>>(feature->PropertyCount);
                    for (var p = 0; p < feature->PropertyCount; ++p)
                    {
                        ...
                        if ((GroundType)(short)key.ToString()[0] != GroundType.Unknown)
                        {
                            GroundType kk = (GroundType)(short)key.ToString()[0];
                            GroundSubType vv = (GroundSubType)(short)value.ToString()[0];
                            if (vv == GroundSubType.Unknown)
                                properties.Add(kk,
                                    new Tuple<string, GroundSubType>(value.ToString(), GroundSubType.Unknown)); 
                            else
                                properties.Add(kk, new Tuple<string, GroundSubType>("", vv));
                        }
                    }

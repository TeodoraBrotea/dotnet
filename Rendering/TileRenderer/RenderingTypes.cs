...
public struct GeoFeature : BaseShape
{
    ...
    public GeoFeature(ReadOnlySpan<Coordinate> c, MapFeatureData feature)
    {
        ...
        var naturalKey = feature.Properties.FirstOrDefault(x => x.Key == GroundType.Natural).Value;
        Type = GeoFeatureType.Unknown;
        if (naturalKey.Item2 != GroundSubType.Unknown)
        {
            if (naturalKey.Item2 == GroundSubType.Fell ||
                naturalKey.Item2 == GroundSubType.Grassland ||
                naturalKey.Item2 == GroundSubType.Heath ||
                naturalKey.Item2 == GroundSubType.Moor ||
                naturalKey.Item2 == GroundSubType.Scrub ||
                naturalKey.Item2 == GroundSubType.Wetland)
            {
                Type = GeoFeatureType.Plain;
            }
            else if (naturalKey.Item2 ==GroundSubType.Wood ||
                     naturalKey.Item2 == GroundSubType.TreeRow)
            {
                Type = GeoFeatureType.Forest;
            }
            else if (naturalKey.Item2 == GroundSubType.BareRock ||
                     naturalKey.Item2 == GroundSubType.Rock ||
                     naturalKey.Item2 == GroundSubType.Scree)
            {
                Type = GeoFeatureType.Mountains;
            }
            else if (naturalKey.Item2 == GroundSubType.Beach ||
                     naturalKey.Item2 == GroundSubType.Sand)
            {
                Type = GeoFeatureType.Desert;
            }
            else if (naturalKey.Item2 == GroundSubType.Water)
            {
...
public struct PopulatedPlace : BaseShape
{
    ...
    public PopulatedPlace(ReadOnlySpan<Coordinate> c, MapFeatureData feature)
    {
        ...
        for (var i = 0; i < c.Length; i++)
            ScreenCoordinates[i] = new PointF((float)MercatorProjection.lonToX(c[i].Longitude),
                (float)MercatorProjection.latToY(c[i].Latitude));
        var name = feature.Properties.FirstOrDefault(x => x.Key == GroundType.Name).Value;
        if (feature.Label.Equals(null))
        {
            ShouldRender = false;
            Name = "Unknown";
        }
    ...
    public static bool ShouldBePopulatedPlace(MapFeatureData feature)
    {
        ...
        foreach (var entry in feature.Properties)
            if (entry.Key == GroundType.Place)
            {
                if (entry.Value.Item2 == GroundSubType.City || entry.Value.Item2 == GroundSubType.Town ||
                    entry.Value.Item2 == GroundSubType.Locality || entry.Value.Item2 == GroundSubType.Hamlet)
                {
                    return true;
                }
            }
...
public struct Border : BaseShape
{
    ...
    public static bool ShouldBeBorder(MapFeatureData feature)
    {
        ...
        foreach (var entry in feature.Properties)
        {
            if (entry.Key == GroundType.Boundary && entry.Value.Item2 == GroundSubType.Administrative)
            {
                foundBoundary = true;
            }
            if (entry.Key == GroundType.AdminLevel && entry.Value.Item2 == GroundSubType.Two)
            {
                foundLevel = true;
            }
